...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        ...
        //指定一个手势需要另一个手势执行失败才会执行，同时触发多个手势使用其中一个手势的解决办法"\，有时手势是相关联的，如单机和双击，点击和长按，点下去瞬间可能只会识别到单击无法识别其他，该方法可以指定某一个 手势，即便自己已经满足条件了，也不会立刻触发，会等到该指定的手势确定失败之后才触发
...
//隐藏整体的scrollview，创建临时的图片做一个消失动画，移除self
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    //根据sourceImagesContainerView找到Cell或者button
    ...
    //将sourceImagesContainerView的cell或者button的frame转化到self坐标上
...
//设置_scrollView的大小和contentsize，设置_scrollView上的SDBrowserImageView的位置和大小，设置索引和保存按钮的大小和位置
- (void)layoutSubviews
{
...
- (void)showFirstImage
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        //点击图片的父视图是UICollectionView时，根据索引indexpath找到UICollectionViewCell作为点击图片的父视图
        UICollectionView *view = (UICollectionView *)self.sourceImagesContainerView;
    ...
    //将点击的图片坐标转换到SDPhotoBrowser坐标系中
    CGRect rect = [self.sourceImagesContainerView convertRect:sourceView.frame toView:self];
    
    //创建tempView目的是为了做一个动画 动画结束后从父视图中移除即可
    UIImageView *tempView = [[UIImageView alloc] init];
